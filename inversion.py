...
import help as hf
import src.MT2DFWD2 as MT
import src.Jacobians as jacos
import matplotlib
...
[xElementLengthMT, zElementLengthMT] = np.meshgrid(
    np.diff(xedgelocationmt), np.diff(zedgelocationmt)
)
...
if sys == "Windows":
    ...
    dir_name = os.path.join("ckpt", "net_" + str(VAE_NO))
elif sys == "Linux":
...
class Encoder(nn.Cell):
    ...
    def construct(self, inputs):
        z_mean, z_log_var = self.mean_model(inputs)
...
if not os.path.exists(os.path.join("output", "inversion", UID)):
    os.makedirs(os.path.join("output", "inversion", UID))
...
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "data_true.png"))
plt.close()
#
...
plot_combined_code(v_array, os.path.join("output", "inversion", UID, "code_start.png"))
...
coor = {
    ...
    "surf_flag": 1,
    "address": os.path.join("output", "inversion", UID, "model_start.png"),
}
hf.Plot2DImage(
    coor,
...
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "data_start.png"))
plt.close()
...
np.savetxt(os.path.join("output", "inversion", UID, "cost.txt"), cost_array, "%.5f")
savemat(
    os.path.join("output", "inversion", UID, "final_model.mat"),
...
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "app_final.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "pha_final.png"))
plt.close()
...
cbar = plt.colorbar()
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "app_delta.png"))
plt.close()
...
cbar = plt.colorbar()
plt.tight_layout()
plt.savefig(os.path.join("output", "inversion", UID, "pha_delta.png"))
plt.close()
